...
namespace game_stuff
{
    public class CharacterStatus : IMoveBattleAttrModel, IBattleUnitStatus
    {
...
#endif
                var aggregate = passiveTraits.Aggregate(Array.Empty<float>(),
            ...
            TwoDVector? moveOp = null)
        {
            ...
            if (!skill.Launch(NowSnipeStep, GetAmmo(), CharEvents, skillAction))
            {
                ok = false;
        ...
        private void OnSnipe(Snipe snipe)
        {
            ...
            if (temp < NowSnipeStep)
            {
                foreach (var stepBuffs in snipe.StepToAddBuff)
                {
                    if (temp >= stepBuffs.Step || stepBuffs.Step > NowSnipeStep) continue;
                    var playingBuffs = stepBuffs.PlayingBuffs.GenBuffs(this);
                    AddPlayingBuff(playingBuffs);


                    // Console.Out.WriteLine(
                    //     $"add snipe buff between {temp} {NowSnipeStep} : {playingBuffs.Aggregate("", (s, x) => s + "|" + x.BuffId)}");
                }
            }
        ...
        public void ResetSnipe()
        {
            ...
            CharEvents.Add(new TickSnipeActionLaunch(SnipeAction.SnipeReset));
#if DEBUG
            ...
            TwoDVector? moveOp, TwoDVector? aim, SkillAction? skillAction)
        {
            ...
            if (releaseSkill != null && releaseSkill.Launch(NowSnipeStep, GetAmmo(), CharEvents))
            {
                SetAct(releaseSkill);
            }
...
//
            ...
            // 当前技能的释放时候 蓄力类技能可以通过按下操作SnipeAction触发
        ...
        private CharGoTickResult ActNowActATick(TwoDVector? operateAim, TwoDVector? operateMove,
            SkillAction? skillAction)
        {
            return NowCastAct == null
                ? CharGoTickResult.CharGoTickResultEmpty
                : GoNowActATick(NowCastAct, operateMove, operateAim, skillAction);
        }
        ...
        private CharGoTickResult OutNowVehicle()
        {
            ...
            {
                NowVehicle.OutAct.Launch(0, 0, CharEvents);
        ...
        {
            var sum = GetAndUseValueBuffs<PowerUpBuff>(out var dec);
            return (1f + sum) / (1 + dec);
        }
        ...
        {
            var takeDamageBuffs = GetAndUseValueBuffs<TakeDamageBuff>(out var dec);
            var multiDecrease = (1f + takeDamageBuffs) / (1f + dec);
        ...
        public float GetAndUseValueBuffs<T>(out float dec) where T : IPlayingBuff, IValueAboutBuff
        {
            var valueAboutBuffs = PlayingBuffs.Values.OfType<T>().OfType<IValueAboutBuff>().ToArray();
            dec = 0f;
            var add = 0f;
            foreach (var playingBuff in valueAboutBuffs)
            {
                var f = playingBuff.GetValue();
                if (playingBuff.ValueMulti > 0f)
                {
                    add += f;
                }
                else
        ...
        public bool GetAndUseExistBuffs<T>() where T : IPlayingBuff
        {
            var ofType = PlayingBuffs.Values.OfType<T>().ToArray();
            var any = ofType.Any();
        ...
        public void UseBuffs<T>() where T : IPlayingBuff
        {
            var ofType = PlayingBuffs.Values.OfType<T>().ToArray();
            foreach (var playingBuff in ofType)
            {
